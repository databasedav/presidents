...
def room_list():
    return [
        {"room": room, "num_players": game.num_players}
        for room, game in room_game_dict.items()
    ]
...
def join_room_as_player(payload) -> None:
    ...
    if game.num_players == 4:
        game.start_round(testing=False)
...
def card_click(payload):
    (lambda sid: sid_game_dict[sid].add_or_remove_card(sid, payload["card"]))(
        request.sid
    )
...
def select_asking_option(payload):
    (
        lambda sid: sid_game_dict[sid].set_selected_asking_option(
            sid, payload["value"]
        )
    )(request.sid)
