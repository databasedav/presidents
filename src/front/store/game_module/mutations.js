// TODO: remove all array hacks after Vue 3 releases
...
export default {
  ...
  alert(state, payload) {
    ...
    state.snackbar = true;
    setTimeout(() => (state.snackbar = false), 1500);
  },
  ...
  remove_asking_option(state, payload) {
    ...
    state.asking_options_arr.splice(state.asking_options_arr.indexOf(payload.value), 1)
    state.asking_options_selected_arr.splice(payload.value, 1, false);
  },
  ...
  // select_hand (state, payload) {
  //   state.stored_hand_lists[payload.id_desc].
  // },
